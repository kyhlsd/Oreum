...
public struct MountainResponse {
    let header: Header
...
public struct MountainInfo: Hashable {
    ...
    public init(id: Int, name: String, address: String, height: Int?, admin: String, adminNumber: String, detail: String, referenceDate: Date?, designationCriteria: String?) {
        self.id = id
        self.name = name
...
extension MountainResponse {
    
    public struct Header {
        let resultCode: String
        let resultMessage: String
        
        public init(resultCode: String, resultMessage: String) {
    ...
    public struct Body {
        ...
        public init(items: Items, numOfRows: Int, page: Int, totalCount: Int) {
            self.items = items
        ...
        public struct Items {
            public let item: [MountainInfo]
            
            public init(item: [MountainInfo]) {
                self.item = item
            }
        }
    }
...
extension MountainResponse {
    public static let dummy: MountainResponse = {
        let items = Body.Items(item: MountainInfo.dummy)
        let body = Body(items: items, numOfRows: 10, page: 1, totalCount: MountainInfo.dummy.count)
        let header = Header(resultCode: "00", resultMessage: "정상 처리되었습니다.")
        return MountainResponse(header: header, body: body)
    }()
}
